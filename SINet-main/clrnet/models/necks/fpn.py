...
from clrnet.models.utils.LKA import Block as SA_Block

from clrnet.models.utils.DCLKA import Block as CSA_Block
...
class FPN(nn.Module):
    def __init__(self,
        ...
        self.catt = CSA_Block(dim=out_channels,mlp_ratio = 3,drop_path=0.3)
    ...
    def forward(self, inputs):
        ...
        #for i, lateral_conv in enumerate(self.lateral_convs):
         #   feat = lateral_conv(inputs[i + self.start_level])
          #  laterals.append(self.att(feat))
            
        for i, lateral_conv in enumerate(self.lateral_convs):
            if i == len(self.lateral_convs)-1:   #  i == 2      
                feat = lateral_conv(inputs[i + self.start_level])
                laterals.append(self.catt(feat))
            else:
